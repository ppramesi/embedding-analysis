...
class VectorService(vector_grpc.vector_services_pb2_grpc.VectorServiceServicer):
    ...
    def Fit(self, request, context):
        ...
        self.pipelines = {
            ...
            # "pca_tsne": (PCA(n_components=50, svd_solver="auto"), TSNE(n_components=3)),
            "pca_umap": (PCA(n_components=100, svd_solver="auto"), umap.UMAP(n_components=3)),
            "umap": umap.UMAP(n_components=3, n_neighbors=20, min_dist=0.25),
            "kernel": KernelPCA(n_components=3, kernel='rbf')  # Change kernel as needed
        }
